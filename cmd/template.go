...
var makeCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for goPath, tplPath := range tasks {
			fileReplace(goPath, tplPath, "github.com/genghongjie/ginbro/boilerplate", "{{.OutPackage}}")
		}
	},
}
...
func fileReplace(goFilepath, tplFilepath, oldString, newString string) error {
	goPath := getGopath()
	goFilepath = path.Join(goPath, "src/github.com/genghongjie/ginbro/boilerplate", goFilepath)
	...
	tplFilepath = path.Join(goPath, "src/github.com/genghongjie/ginbro/template", tplFilepath)
