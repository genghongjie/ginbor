...
func (p *ParseEngine) SaveFlagsToUserConfigFile() error {
	...
	tplPath := path.Join(goPath, "src", "github.com/genghongjie/ginbro", "tpl/cobra.ginbro.yaml")
...
func (p *ParseEngine) GenerateProjectCode() {
	...
	for _, m := range []string{"config", "handlers", "models", "tasks", "service"} {
		p.makeModuleDir(m)
	}
	//copy file
	...
	for _, tableSchema := range p.Tables {
		...
		if err := tableSchema.generateService(); err != nil {
			log.Println(err)
		}
	...
	fileName := fmt.Sprintf("service/service_test.go")
	parseTemplate("tpl/service_test.template.tpl", p.OutPackage, fileName, p)
...
func (p *ParseEngine) copyStaticSwaggerFolder() {
	...
	for _, vv := range tasks {
		src := path.Join(goPath, "src", "github.com/genghongjie/ginbro/boilerplate", vv)
